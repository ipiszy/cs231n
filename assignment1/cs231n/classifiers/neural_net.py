...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # L = L1 + L2, where L1 = prob loss and L2 = regularization
    # Then,
    # dL / dW2 = dL1 / dW2 + dL2 / dW2 = dL1 / dW2 + W2 * reg, and
    # dL / dW1 = dL1 / dW1 + W1 * reg, where
    # dL1 / dW2 = (dL1 / dS) * (dS / dW2)
    # dL2 / dW1 = (dL1 / dS) * (dS / dS1) * (dS1 / dW1)
    
    # dL1 / dS
    ds = fj_exp / loss_sum[:, None]
    ds[np.arange(N), y] -= 1
    ds /= N
    
    # dL1 / dW2
    grads['W2'] = np.dot(s1.T, ds)  # (H, C)
    # dL1 / db2
    grads['b2'] = np.sum(ds, axis = 0)
    
    # dL1 / dS1
    ds1 = np.dot(ds, W2.T)  # (N, H)
    ds1[s1 == 0] = 0
    
    # dL1 / dW1
    grads['W1'] = np.dot(X.T, ds1) # (D, H)
    # dL1 / db1
